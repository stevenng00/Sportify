...
string issueBy = (dRow != null) ? dRow["Issued_By"].ToString() : UCA.CustomButton.CustomUtility.Get_GridViewValue("Issued_By", sgv.Rows[rno]);
string combineEmail = "";

List<string> receiverEmails = new List<string>();
List<string> receiverNames = new List<string>();

DataTable approvalTable = dbUtility.FillDataTable(@"SELECT DISTINCT Employee_Profile.Employee_Name AS Display_Name, Employee_Profile.Email FROM Employee_Profile INNER JOIN 
                                                                                (
                                                                                SELECT
                                                                                    Approval_PIC.Employee_Profile_ID
                                                                                FROM Approval_PIC
                                                                                INNER JOIN Approval_Profile ON Approval_Profile.Approval_Profile_ID = Approval_PIC.Approval_Profile_ID
                                                                                INNER JOIN Purchase_Order ON CAST(Purchase_Order.Amount*Purchase_Order.Exchange_Rate AS Decimal(20,2)) BETWEEN Approval_Profile.Min_Range AND Approval_Profile.Max_Range
                                                                                WHERE Purchase_Order.Purchase_Order_ID = '" + POID + @"'
                                                                                        AND Approval_Profile.Status = 'Active'
                                                                                        AND Approval_Profile.Type = 'Purchase Order'
    ...
    if (approvalTable.Rows.Count > 0)
    {
        foreach (DataRow approvalRow in approvalTable.Rows)    
        {
            if (string.IsNullOrEmpty(combineEmail))
                combineEmail = approvalRow["Email"].ToString();
            else
                combineEmail = combineEmail + "," + approvalRow["Email"].ToString();
        }

        subject = POType + " PO " + PONo + " Require Approval";

        content = "Dear PIC, <br/><br/>";
        content = content + "The following purchase order requires your approval:<br/><br/>";
        ...
        content = content + "<br/>Issue By : " + issueBy;

        content = content + "<br/><br/>";
        content = content + "Please login to the system for more details.";
